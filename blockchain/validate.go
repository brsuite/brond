...
func CheckTransactionSanity(tx *bronutil.Tx) error {
	...
	// as a bronees.  One brocoin is a quantity of bronees as defined by the
	// BroneesPerBrocoin constant.
	var totalBronees int64
	for _, txOut := range msgTx.TxOut {
		...
		if bronees > bronutil.MaxBronees {
		...
		if totalBronees > bronutil.MaxBronees {
			str := fmt.Sprintf("total value of all transaction "+
				"outputs is %v which is higher than max "+
				"allowed value of %v", totalBronees,
				bronutil.MaxBronees)
			return ruleError(ErrBadTxOutValue, str)
		}
	}
...
// the brocoins and therefore allowed to spend them.  As it checks the inputs,
...
func CheckTransactionInputs(tx *bronutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// brocoin is a quantity of bronees as defined by the
		// BroneesPerBrocoin constant.
		originTxBronees := utxo.Amount()
		if originTxBronees < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", bronutil.Amount(originTxBronees))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxBronees > bronutil.MaxBronees {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				bronutil.Amount(originTxBronees),
				bronutil.MaxBronees)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		lastBroneesIn := totalBroneesIn
		totalBroneesIn += originTxBronees
		if totalBroneesIn < lastBroneesIn ||
			totalBroneesIn > bronutil.MaxBronees {
			str := fmt.Sprintf("total value of all transaction "+
				"inputs is %v which is higher than max "+
				"allowed value of %v", totalBroneesIn,
				bronutil.MaxBronees)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
	}
	...
	var totalBroneesOut int64
	for _, txOut := range tx.MsgTx().TxOut {
		totalBroneesOut += txOut.Value
	}

	// Ensure the transaction does not spend more than its inputs.
	if totalBroneesIn < totalBroneesOut {
		str := fmt.Sprintf("total value of all transaction inputs for "+
			"transaction %v is %v which is less than the amount "+
			"spent of %v", txHash, totalBroneesIn, totalBroneesOut)
	...
	txFeeInBronees := totalBroneesIn - totalBroneesOut
	return txFeeInBronees, nil
}
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block *bronutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
	...
	expectedBroneesOut := CalcBlockSubsidy(node.height, b.chainParams) +
		totalFees
	if totalBroneesOut > expectedBroneesOut {
		str := fmt.Sprintf("coinbase transaction for block pays %v "+
			"which is more than expected value of %v",
			totalBroneesOut, expectedBroneesOut)
