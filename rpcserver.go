...
import (
	...
	"github.com/brsuite/brond/database"
	"github.com/brsuite/brond/mempool"
	"github.com/brsuite/brond/mining"
	"github.com/brsuite/brond/mining/cpuminer"
...
var (
	...
	// data.
	gbtCoinbaseAux = &bronjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &bronjson.RPCError{
	...
	ErrRPCNoWallet = &bronjson.RPCError{
		Code:    bronjson.ErrRPCNoWallet,
...
// should ask a connected instance of bronwallet.
...
// not needed.
func internalRPCError(errStr, context string) *bronjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return bronjson.NewRPCError(bronjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *bronjson.RPCError {
	return bronjson.NewRPCError(bronjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *bronjson.RPCError {
	return bronjson.NewRPCError(bronjson.ErrRPCNoTxInfo,
...
// These commands will be implemented in bronwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > bronutil.MaxBronees {
			return nil, &bronjson.RPCError{
				Code:    bronjson.ErrRPCType,
		...
		bronees, err := bronutil.NewAmount(amount)
		if err != nil {
...
func handleDebugLevel(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		return nil, &bronjson.RPCError{
			Code:    bronjson.ErrRPCInvalidParams.Code,
			Message: err.Error(),
		}
	}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []bronjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]bronjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &bronjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []bronjson.Vout {
	voutList := make([]bronjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout bronjson.Vout
		vout.N = uint32(i)
		vout.Value = bronutil.Amount(v.Value).ToBRON()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*bronjson.TxRawResult, error) {
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.DecodeRawTransactionCmd)
	...
	txReply := bronjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.DecodeScriptCmd)
	...
	reply := bronjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !s.cfg.ChainParams.GenerateSupported {
		return nil, &bronjson.RPCError{
			Code: bronjson.ErrRPCDifficulty,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &bronjson.RPCError{
				Code:    bronjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*bronjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result bronjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = bronjson.Bool(peer.Connected())
		...
		addrs := make([]bronjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr bronjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		return nil, &bronjson.RPCError{
			Code:    bronjson.ErrRPCBlockNotFound,
	...
	blockReply := bronjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]bronjson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &bronjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		SoftForks: &bronjson.SoftForks{
			Bip9SoftForks: make(map[string]*bronjson.Bip9SoftForkDescription),
		},
	}
	...
	chainInfo.SoftForks.SoftForks = []*bronjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.SoftForks.Bip9SoftForks[forkName] = &bronjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &bronjson.RPCError{
			Code:    bronjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := bronjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a bronjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*bronjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]bronjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := bronjson.GetBlockTemplateResultTx{
	...
	reply := bronjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *bronjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &bronjson.RPCError{
			Code:    bronjson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &bronjson.RPCError{
			Code:    bronjson.ErrRPCClientInInitialDownload,
...
// chainErrToGBTErrString converts an error returned from bronchain to a string
...
func handleGetBlockTemplateProposal(s *rpcServer, request *bronjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &bronjson.RPCError{
			Code: bronjson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &bronjson.RPCError{
			Code:    bronjson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*bronjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bronjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &bronjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToBRON(),
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &bronjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := bronjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := bronjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &bronjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bronjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*bronjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &bronjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &bronjson.RPCError{
				Code: bronjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &bronjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         bronutil.Amount(value).ToBRON(),
		ScriptPubKey: bronjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]bronjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]bronjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]bronjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		vinEntry := bronjson.VinPrevOut{
			Txid:     prevOut.Hash.String(),
			Vout:     prevOut.Index,
			Sequence: txIn.Sequence,
			ScriptSig: &bronjson.ScriptSig{
				Asm: disbuf,
				Hex: hex.EncodeToString(txIn.SignatureScript),
			},
		}
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &bronjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     bronutil.Amount(originTxOut.Value).ToBRON(),
			}
		}
	}
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*bronjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]bronjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = bronjson.ErrRPCTxAlreadyInChain

			default:
				code = bronjson.ErrRPCTxRejected
			}
		}
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.ValidateAddressCmd)

	result := bronjson.ValidateAddressChainResult{}
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*bronjson.VerifyMessageCmd)
...
//
// NOTE: This is a brsuite extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]bronjson.VersionResult{
		"brondjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, bronjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *bronjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(bronjson.Error); ok &&
			jerr.ErrorCode == bronjson.ErrUnregisteredMethod {

			parsedCmd.err = bronjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = bronjson.NewRPCError(
			bronjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *bronjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *bronjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*bronjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return bronjson.MarshalResponse(id, result, jsonErr)
}
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if jsonErr == nil {
		...
		//
		// Brond does not respond to any request without and "id" or "id":null,
