...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt bronutil.Amount) (*bronec.PrivateKey, *wire.OutPoint, []byte, error) {

	// Create a fresh key, then send some coins to an address spendable by
	// that key.
	key, err := bronec.NewPrivateKey(bronec.S256())
	if err != nil {
...
func TestBIP0113Activation(t *testing.T) {
	...
	brondCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, brondCfg)
	if err != nil {
	...
	const outputValue = bronutil.BroneesPerBrocoin
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numBronees bronutil.Amount, timeLock int32,
